...
int main() {
    ...
    while(true) {
        ...
        try {
            edizione = LibroEdizione::fromRedisStream(reply, 0, 0);
        }
        ...
        sprintf(query, "INSERT INTO LibroEdizione (ISBN, titolo, pagine, casaEditrice, autore, genere) VALUES ('%s', '%s', %d, '%s', %d, '%s')", 
                        edizione->ISBN.c_str(), edizione->titolo.c_str(), edizione->pagine, 
                        edizione->casa_editrice.c_str(), edizione->autore_id, edizione->genere.c_str());
