...
int main() {
    ...
    while(true) {
        ...
        try {
            sanzione = Sanzione::fromRedisStream(reply, 0, 0);
        }
        ...
        sprintf(query, "INSERT INTO Sanzione (costo, motivazione, data, bibliotecario, utente) VALUES (%f, \'%s\', \'%s\', %d, %d)", 
                        sanzione->costo, sanzione->motivazione.c_str(), sanzione->data.c_str(),
