...
int main() {
    ...
    while(true) {
        ...
        try {
            richiesta = RichiestaPrestito::fromRedisStream(reply, 0, 0);
        }
        ...
        sprintf(query, "INSERT INTO RichiestaPrestito (utente, libro, dataInizio, dataFine, istante) VALUES (%d, %d, \'%s\', \'%s\', CURRENT_TIMESTAMP)", 
